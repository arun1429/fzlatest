import {Animated, Dimensions, GestureResponderEvent, ImageURISource, LayoutChangeEvent, Slider, Text, TextStyle, TouchableOpacity, TouchableOpacityProps, TouchableWithoutFeedback, View, ViewProps, ViewStyle} from 'react-native';
import {Audio, Video} from 'expo-av';
import {Ionicons} from '@expo/vector-icons';
import {FullscreenEnterIcon, FullscreenExitIcon, PauseIcon, PlayIcon, ReplayIcon, Spinner} from '../../../assets/assets/icons';
...
const IOS_THUMB_IMAGE = require('../../../assets/assets/thumb.png');
// eslint-disable-next-line @typescript-eslint/no-var-requires
const IOS_TRACK_IMAGE = require('../../../assets/assets/track.png');
const SLIDER_COLOR = '#D81F26';
...
const defaultProps = {
  ...
  switchToPortrait: () => {
    ScreenOrientation.lockAsync(ScreenOrientation.Orientation.POTRAIT);
  },
...
const VideoPlayer = props => {
  ...
  const Control = ({callback, center, children, transparent = false, ...otherProps}) => (
    <TouchableOpacity
      {...otherProps}
    ...
    <TouchableWithoutFeedback onPress={toggleControls}>
      <View style={{backgroundColor: videoBackground}}>
        <Video
          ...
          {...otherVideoProps}
        />
        {seekState !== SeekStates.Seeking && (playbackState === PlaybackStates.Playing || playbackState === PlaybackStates.Paused) && (
          <Animated.View
            ...
            <Control center={true} callback={_back}>
              {/* Due to rendering, we have to split them */}
              <Ionicons name="ios-close" size={60} color="#000" />
            </Control>
          </Animated.View>
        )}
        {/* Spinner */}
        {/* Due to loading Animation, it cannot use CenteredView */}
        {((playbackState === PlaybackStates.Buffering && Date.now() - lastPlaybackStateUpdate > BUFFERING_SHOW_DELAY) || playbackState === PlaybackStates.Loading) && (
          <View
            style={{
              position: 'absolute',
              left: (videoWidth - centeredContentWidth) / 2,
        ...
        <Animated.View
          ...
          <Text style={[textStyle, {backgroundColor: 'transparent', marginLeft: 5}]}>{getMMSSFromMillis(playbackInstancePosition)}</Text>

          {/* Seek bar */}
          <TouchableWithoutFeedback onLayout={onSliderLayout} onPress={onSeekBarTap}>
            <Slider style={{marginRight: 10, marginLeft: 10, flex: 1}} thumbTintColor={sliderColor} minimumTrackTintColor={sliderColor} trackImage={iosTrackImage} thumbImage={iosThumbImage} value={getSeekSliderPosition()} onValueChange={onSeekSliderValueChange} onSlidingComplete={onSeekSliderSlidingComplete} disabled={playbackState === PlaybackStates.Loading || playbackState === PlaybackStates.Ended || playbackState === PlaybackStates.Error || controlsState !== ControlStates.Shown} />
          </TouchableWithoutFeedback>

          {/* Duration display */}
          <Text style={[textStyle, {backgroundColor: 'transparent', marginRight: 5}]}>{getMMSSFromMillis(playbackInstanceDuration)}</Text>
